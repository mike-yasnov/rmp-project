    ...
    route("/api/quotes") {
        get {
            ...
            call.respond(quotes.map {
                QuoteResponse(ticker = it.ticker, price = it.price, volume = it.volume, timestamp = it.timestamp.toString())
            })
        }
        ...
        get("/{ticker}") {
            ...
            call.respond(QuoteResponse(ticker = quote.ticker, price = quote.price, volume = quote.volume, timestamp = quote.timestamp.toString()))
        }
        ...
        get("/{ticker}/candles") {
            ...
            call.respond(candles.map {
                CandleResponse(ticker = it.ticker, open = it.open, high = it.high, low = it.low, close = it.close, volume = it.volume, timestamp = it.timestamp.toString())
            })
        }
    }
}
